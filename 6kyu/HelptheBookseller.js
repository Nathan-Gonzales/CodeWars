...
function stockList(listOfArt, listOfCat){
    ...
    const allBooks = {}
    listOfCat.forEach(category => allBooks[category] = 0)
    
    listOfArt.forEach(books => {
      allBooks[books.substring(0, 1)] += Number(books.substring(books.indexOf(' ')))
    })
    
    let result = ""
    for(const category in allBooks){
      if(!isNaN(allBooks[category])){
        result += `(${category} : ${allBooks[category]})`
        if(category !== listOfCat[listOfCat.length - 1]){
          result += ' - '
        }
      }
